...
EdgeOne Pages Functions作为用户访问的入口端点（即上文提到的端点），收到请求后根据HTTP请求的Path进行处理，区分横屏、竖屏、自适应三类，在内部生成一个随机数，范围硬编码在代码里，图源我没有设置防盗链故未作处理（如果你要自己部署自定义后端端点的API请自行解决好防盗链问题），更多细节详见我发布的源码，见此处：[https://github.com/wdfqx/edgeone_randomimg_api](https://github.com/wdfqx/edgeone_randomimg_api)
